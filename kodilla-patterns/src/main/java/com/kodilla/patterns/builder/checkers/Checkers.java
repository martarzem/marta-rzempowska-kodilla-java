...
public class Checkers {
    ...
    private Checkers(Board board, String playerOne, String playerTwo) { //prywatny konstruktor - może być wywołany jedynie
        this.board = board; //z klasy CheckersBuilder
    ...
    public static class CheckersBuilder { //klasa wewnętrzna
        ...
        //tworzy nową figurę i wstawią ją na planszy w miejscu o współrzędnych [x][y]
        //sprawdza także, czy wartści x i y mieszczą się w założonych przedziałach
        ...
        //przed utworzeniem obiektu dokunujesz szeregu walidacji
        public Checkers build() {
            if (playerOne.length() == 0) { //sprawdza, czy zainicjowano wartość zmiennej playerOne...
                throw new IllegalStateException("There is no player One specified!");
            }
            if (playerTwo.length() == 0) { //...i playerTwo
                throw new IllegalStateException("There is no player Two specified");
            }
            //sprawdza, czy na planszy znajduje się przynajmniej po jednej figurze z każdego koloru
